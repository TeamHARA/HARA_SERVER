...
describe("PATCH /worry/with", () => {
  ...
  it("올바른 응답", async () => {
    const response = await request(app).patch("/worry/with").send({
      worryWithId: createdWithWorry.id,
      chosenOptionId: createdOption.id,
    });
    expect(response.statusCode).toBe(200);
    expect(response.body).toMatchObject({
      status: 200,
      success: true,
      message: "나의고민 최종결정 성공",
    });
  });
});

describe("GET /worry/with/list/0", () => {
  it("올바른 응답", async () => {
    const response = await request(app).get("/worry/with/list/0");
    expect(response.statusCode).toBe(200);
    expect(response.body.message).toEqual("함께고민 조회 성공");
  });
});

describe("GET /worry/with/list/1", () => {
  it("올바른 응답", async () => {
    const response = await request(app).get("/worry/with/list/1");
    expect(response.statusCode).toBe(200);
    expect(response.body.message).toEqual("함께고민 조회 성공");
  });
});
